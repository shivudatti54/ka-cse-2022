...
-   `wait(S)` (also historically called `P(S)` from the Dutch *Proberen*, meaning "to test"):
    ```c
    while (S <= 0)
...
-   `signal(S)` (also called `V(S)` from *Verhogen*, meaning "to increment"):
    ```c
    S++;
    ```
...
```
*Note: The order of `wait` calls is crucial. Swapping them can lead to deadlock.*
