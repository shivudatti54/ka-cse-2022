...
_Denies everything by default. Only allows scripts from self (with a nonce for specific inline code), styles from self, images from self, and API connections to self._
...
_Allows resources from self and specific trusted CDNs. Allows data URLs for images (e.g., inline SVGs)._
...
1.  **CSP is a whitelist mechanism.** Remember it defines what is _allowed_, not what is _blocked_.
