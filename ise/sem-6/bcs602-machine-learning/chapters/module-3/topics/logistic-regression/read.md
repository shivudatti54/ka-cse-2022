...
1.  **Understand the "Why":** Don't just memorize formulas. Understand *why* we need polynomial regression (non-linear data) and *why* logistic regression uses a sigmoid function (to output probabilities).
...
4.  **Interpret the Coefficients:** In linear regression, `β₁` is the change in `y` for a one-unit change in `x`. In logistic regression, `β₁` is the change in the *log-odds* of the outcome for a one-unit change in `x`. You may need to transform this into an **odds ratio** (`e^{β₁}`).
