...
The `WHERE` clause filters rows _before_ they are grouped. The `HAVING` clause filters groups _after_ the `GROUP BY` has been applied. It is used to apply conditions to aggregate values.
