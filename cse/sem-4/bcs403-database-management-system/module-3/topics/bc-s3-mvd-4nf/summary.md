...
- **Multivalued Dependency (MVD)**: A constraint where attribute B has multiple independent values for each value of attribute A. Denoted as A � B, meaning "A multidetermines B."

- **Fourth Normal Form (4NF)**: A relation R is in 4NF if for every non-trivial MVD A � B in R, A is a candidate key of R.

- **Trivial MVD**: A � B is trivial if B ⊆ A or A ∪ B contains all attributes of R. Trivial MVDs do not cause redundancy.

- **Non-Trivial MVD**: A � B is non-trivial if B is not a subset of A and A ∪ B does not contain all attributes. Non-trivial MVDs cause redundancy and violate 4NF.

## Important Formulas and Theorems

- **4NF Condition**: For all non-trivial MVDs A � B, A must be a candidate key.

- **Decomposition Algorithm**: For MVD A � B, decompose into:
- R1(A, B)
- R2(A, (R - A - B))
- **Symmetry Property**: If A � B holds, then A � (R - A - B) also holds.
...
- Symmetry property of MVDs means if A � B exists, A � (all other attributes excluding A and B) also exists
