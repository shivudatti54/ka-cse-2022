#!/usr/bin/env python3
"""
VTU Content Generator - MiniMax API (M2.5)

Generates high-quality educational content for VTU topics using MiniMax-M2.5.
...
Usage:
  python3 content_generator_minimax.py topic-paths.txt --start-from 101
  python3 content_generator_minimax.py topic-paths.txt --resume
"""
...
MAX_TOKENS = 8192
TEMPERATURE = 0.3
PROGRESS_FILE = "minimax_progress.json"
...
def extract_text(response: dict) -> str:
    ...
    # MiniMax might use reasoning format (like DeepSeek)
    if not content:
...
# ── Main ───────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="VTU Content Generator - MiniMax (M2.5)")
    ...
    # Print header
    print(f"\n{'='*80}")
    print("VTU CONTENT GENERATOR - MINIMAX (M2.5)")
    print(f"{'='*80}")
