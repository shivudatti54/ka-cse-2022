#!/usr/bin/env python3
"""Test Cerebras API - check available models and rate limits."""
...
def test_completion(model_id="llama-3.3-70b"):
    ...
    try:
        ...
        if content:
            ...
            # Check time per token
            time_per_token = data.get("time_per_token_ms")
            if time_per_token:
                print(f"\n⚡ Performance:")
                print(f"  Time per token: {time_per_token}ms")
                print(f"  Tokens per second: {1000/time_per_token:.1f}")
...
def main():
    ...
    if models:
        ...
        for model_id in ["llama-3.3-70b", "qwen-3-235b-a22b-instruct-2507"]:
    ...
    print("SUMMARY")
    print("=" * 80)
    print("Cerebras offers 1M tokens/day FREE tier")
    print("Known for extremely fast inference (450-1800 tokens/sec)")
    print("=" * 80 + "\n")
