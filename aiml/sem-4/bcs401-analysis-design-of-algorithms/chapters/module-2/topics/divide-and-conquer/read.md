...
*   **Non-overlapping Subproblems**: The subproblems created should be independent of each other. (Note: This is a key difference from Dynamic Programming, which handles *overlapping* subproblems).
...
*   `F(n)` is the cost of the *divide* and *combine* steps.
...
*   **Combine:** The solution from the subproblem *is* the solution to the original problem; no combination is needed.
