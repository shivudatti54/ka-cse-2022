...
The goal of **image restoration** is to estimate the original, uncorrupted image from its degraded version. Unlike image *enhancement*, which is subjective and aims to make an image look better, restoration is an objective process that attempts to reverse known or estimated degradation using mathematical models.
