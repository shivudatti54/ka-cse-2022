...
```
    ...
    +-------------+      +-----------------+      | (Trusted)     |      |  Store)   |
                                                   +---------------+      +-----------+
```
*This simple diagram shows data flowing from an untrusted source (Internet) through a web server (crossing a trust boundary) to trusted internal components. Each arrow represents a data flow and a potential entry point for analysis.*
...
**Technique:** Apply the STRIDE model *per element* in your DFD. For example, ask: "How could an attacker spoof identity at this data flow? How could they tamper with data at this process?"
