...
Before any data page is written to the database on disk, the log records corresponding to the changes on that page *must* be written to stable storage (the log). This ensures that if a crash occurs, we have a record of the change to either redo it or undo it.
...
During recovery, the system only needs to consider transactions that were active *at the time of the last checkpoint* or that started *after* the checkpoint.
...
5.  **Participant P1 crashes** *after* writing `<ready T>` but *before* receiving the `commit` message.
...
*   **Understand Blocking:** Be able to explain *why* a participant in 2PC becomes blocked after a crash and what it must do to recover. This is a classic exam question.
