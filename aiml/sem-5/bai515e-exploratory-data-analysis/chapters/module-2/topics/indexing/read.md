...
4.  **Think in Terms of Big-O:** Remember that hash indexes offer O(1) lookup _on average_, but worst-case can be O(n) if all keys collide (highly unlikely with a good hash function). B+ Trees offer a predictable O(log n) performance.
