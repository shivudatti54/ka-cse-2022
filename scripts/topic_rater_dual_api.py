...
def get_topic_context(topic_path: str) -> dict:
    ...
    Returns dict with keys: subject, module, topic, semester, subject_slug, topic_slug
    """
    topic_dir = Path(topic_path)
    topic_slug = topic_dir.name

    # Walk up: topic -> topics -> module -> chapters -> subject -> semester
    ...
    # Try to read _index.json files for better names
    sibling_topics = []
    try:
        # Module _index.json (2 levels up from topic)
        module_dir = topic_dir.parent.parent  # topics -> module
        ...
        if module_index.exists():
            ...
            # Find topic title and collect siblings
        ...
        # Subject _index.json (4 levels up from topic)
        subject_dir = module_dir.parent.parent  # chapters -> subject
        context['subject_slug'] = subject_dir.name
        ...
        context['semester'] = sem_dir.name.replace('-', ' ').title()

    except Exception:
        # Fallback: parse from path segments
        for i, part in enumerate(parts):
            if part.startswith('sem-'):
                context['semester'] = part.replace('-', ' ').title()
            elif re.match(r'^[a-z]{2,5}\d{3}', part):
                context['subject_slug'] = part
                context['subject'] = _slug_to_title(part)
            elif part.startswith('module-'):
                context['module'] = part.replace('-', ' ').title()

    context['sibling_topics'] = sibling_topics
    return context
...
RATING_PROMPT = """\
You are an expert educational content reviewer for a university engineering study app.

You will be given all files from a single topic directory, along with the SUBJECT and MODULE context. Rate the topic on THREE dimensions:

1. **Subject Relevance Rating** (1-10): Does the content actually match the stated subject and topic?
   - Check the SUBJECT, MODULE, and OTHER TOPICS IN THIS MODULE fields for context
   - The sibling topics tell you what domain area this module covers
   - Example: If topic is "Random Access" and siblings are "Error Control, Flow Control, Framing" → this is networking, NOT array indexing
   - Score 1-3 if the content is about a COMPLETELY DIFFERENT subject/domain
   - Score 4-6 if partially relevant but has significant off-topic sections
   - Score 7-10 if properly aligned with the stated subject, module domain, and topic

2. **Coverage Rating** (1-10): How well does the content cover the topic?
...
3. **Content Quality Rating** (1-10): How good is the content itself?
...
{
  "subject_relevance_rating": <1-10>,
  "subject_relevance_remarks": "<brief explanation - flag if content is about wrong subject>",
...
def load_topic_content(topic_path: str) -> str:
    ...
    # Extract subject/module context from directory structure
    ctx = get_topic_context(topic_path)
    siblings = ctx.get('sibling_topics', [])
    siblings_str = ", ".join(siblings) if siblings else "(not available)"

    parts.append(f"=== SUBJECT: {ctx['subject']} ===")
    parts.append(f"=== MODULE: {ctx['module']} ===")
    parts.append(f"=== TOPIC: {ctx['topic']} ===")
    parts.append(f"=== SEMESTER: {ctx['semester']} ===")
    parts.append(f"=== OTHER TOPICS IN THIS MODULE: {siblings_str} ===")
    parts.append(f"Path: {topic_path}")
    parts.append(f"\nIMPORTANT: All content in this topic MUST be about \"{ctx['topic']}\" "
                 f"as it relates to the subject \"{ctx['subject']}\". "
                 f"The other topics in the same module ({siblings_str}) indicate the area/domain. "
                 f"If the content is about a completely different subject or domain, "
                 f"flag it as subject mismatch and give a low subject_relevance_rating.\n")
...
def generate_html_report(report: dict, html_path: Path):
    ...
    totals: dict[str, dict] = {a: {"rel": [], "cov": [], "qual": []} for a in api_names}
    error_count = 0
    total_rated = 0
    ...
    for key in sorted(topics.keys(), key=lambda x: int(x)):
        ...
        if subject and subject not in by_subject:
            by_subject[subject] = {a: {"rel": [], "cov": [], "qual": []} for a in api_names}
        ...
        for api in api_names:
            ...
            if not r or "error" in r:
                ...
                continue
            total_rated += 1
            rel = r.get("subject_relevance_rating")
            cov = r.get("coverage_rating")
            qual = r.get("content_quality_rating")
            if isinstance(rel, (int, float)):
                totals[api]["rel"].append(rel)
                if subject:
                    by_subject[subject][api]["rel"].append(rel)
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    for api in api_names:
        ...
        ra = _avg(totals[api]["rel"])
        ca = _avg(totals[api]["cov"])
        qa = _avg(totals[api]["qual"])
        w(f'<div class="card"><div class="label">{_esc(api)} avg (Rel / Cov / Qual)</div>')
        w(f'  <div class="value rating">{ra} / {ca} / {qa}</div>')
    ...
    for api in api_names:
        ...
        w(f'  <th>{_esc(api)}<br><span style="font-weight:400;text-transform:none;font-size:0.7rem">{_esc(short_model)}</span><br>Rel / Cov / Qual</th>')
    w("  <th>Details</th>")
    w("</tr></thead>")
    w("<tbody>")
    ...
    for key in sorted(topics.keys(), key=lambda x: int(x)):
        ...
        for api in api_names:
            ...
            if not r or "error" in r:
                ...
                cov = r.get("coverage_rating")
                qual = r.get("content_quality_rating")
                w(f'  <td><span class="r {_rating_cls(rel)}">{rel if rel is not None else "?"}</span>'
                  f' / <span class="r {_rating_cls(cov)}">{cov if cov is not None else "?"}</span>'
        ...
        for api in api_names:
            ...
            rel_rem = r.get("subject_relevance_remarks", "")
            cov_rem = r.get("coverage_remarks", "")
            qual_rem = r.get("content_quality_remarks", "")
            w(f'      <p class="remark"><strong>Relevance ({r.get("subject_relevance_rating", "?")}/10):</strong> {_esc(rel_rem)}</p>')
...
async def main():
    ...
    print(f"{'='*70}")
    print(f"  {'#':>3}  {'Topic':<40} {'API':>7} {'Model':<25} {'Rel':>4} {'Cov':>4} {'Qual':>4}")
    print(f"  {'-'*3}  {'-'*40} {'-'*7} {'-'*25} {'-'*4} {'-'*4} {'-'*4}")

    rel_totals = {api: [] for api in selected_apis}
    ...
    for idx in sorted(by_topic.keys()):
        ...
        for api_name, data in entry["ratings"].items():
            ...
            model = data["model"]
            if "error" in rating:
                print(f"  {idx:>3}  {name:<40} {api_name:>7} {model:<25} {'ERR':>4} {'ERR':>4} {'ERR':>4}")
            else:
                rel = rating.get("subject_relevance_rating", "?")
                cov = rating.get("coverage_rating", "?")
                qual = rating.get("content_quality_rating", "?")
                print(f"  {idx:>3}  {name:<40} {api_name:>7} {model:<25} {rel:>4} {cov:>4} {qual:>4}")
                if isinstance(rel, (int, float)):
                    rel_totals[api_name].append(rel)
                if isinstance(cov, (int, float)):
    ...
    print(f"\n  {'AVERAGES':>3}  {'':40}", end="")
    for api_name in selected_apis:
        rel_avg = sum(rel_totals[api_name]) / len(rel_totals[api_name]) if rel_totals[api_name] else 0
        ...
        print(f"\n       {'':40} {api_name:>7} {'avg':<25} {rel_avg:>4.1f} {cov_avg:>4.1f} {qual_avg:>4.1f}", end="")
