#!/usr/bin/env python3
"""
VTU Content Generator - MiniMax Parallel

Generates content for VTU topics using MiniMax-M2.5 with parallel processing.
...
def print(*args, **kwargs):
    """Timestamp every log line with local current time."""
...
TEMPERATURE = 0.3
DEFAULT_WORKERS = 25
PROGRESS_FILE = "minimax_parallel_progress.json"
MD_MAX_ATTEMPTS = 2
...
- Students: CUET-qualified, bright undergraduates at colleges like Hansraj, SRCC, Miranda House, St. Stephen's, Hindu College
...
- DU students are competitive — content quality must match top university standards
...
Length: 1500-3000 words, BSc (Hons) level
===PURPOSE_MD===
# Learning Objectives
...
def call_api_with_heartbeat(messages: list, topic_desc: str) -> dict:
    """Call MiniMax API with timeout and heartbeat monitoring."""
    payload = json.dumps({
...
def extract_text(response: dict) -> str:
    ...
    try:
        ...
        # MiniMax returns content in 'content' field primarily
        ...
        # MiniMax wraps responses in <think>...</think> tags - remove them
        if '<think>' in text and '</think>' in text:
            # Extract content after </think> tag
            think_end = text.find('</think>')
            if think_end != -1:
                text = text[think_end + 8:].strip()  # 8 = len('</think>')
                print(f"    DEBUG: Stripped <think> tags, now {len(text)} chars")
...
def _clean_json_payload(content: str) -> str:
    """Normalize model output to improve JSON parsing reliability."""
    cleaned = content.strip()
    if cleaned.startswith("```"):
        print("    DEBUG: Removing markdown fences")
    ...
    # Keep only the outermost JSON object if extra text is present.
...
Return ONLY valid JSON as specified."""
    ...
    content = _clean_json_payload(content)
...
def get_topic_context(topic_path: str) -> dict:
    ...
    # Read module _index.json for topic title, module title, and sibling topics
    sibling_topics = []
...
def process_topic(topic_path: str, topic_num: int, total: int) -> dict:
    ...
    try:
        # Call 1: Markdown files (retry if incomplete)
        markdown_files = {}
        for attempt in range(1, MD_MAX_ATTEMPTS + 1):
            if attempt > 1:
                print(
                    f"  [{topic_num}:MD {ctx['topic']}] Retrying markdown generation "
                    f"({attempt}/{MD_MAX_ATTEMPTS})"
                )
            markdown_files = generate_markdown_batch(ctx, topic_num)
            if len(markdown_files) >= 3:
                break
            if attempt < MD_MAX_ATTEMPTS:
        ...
        # Call 2: Assessment files (retry on malformed/partial JSON response)
        read_content = markdown_files.get('read.md', '')
        assessment_files = {}
        if read_content:
            for attempt in range(1, ASSESSMENT_MAX_ATTEMPTS + 1):
                if attempt > 1:
                    print(
                        f"  [{topic_num}:JSON {ctx['topic']}] Retrying assessment generation "
                        f"({attempt}/{ASSESSMENT_MAX_ATTEMPTS})"
                    )
                assessment_files = generate_assessment_batch(ctx, read_content, topic_num)
                if len(assessment_files) >= 6:
                    break
        ...
        result["files_written"] = len(written_files)

        result["success"] = result["files_written"] >= 7
        if not result["success"] and result["files_written"] >= 3 and len(assessment_files) < 6:
            result["error"] = "Assessment JSON invalid/incomplete after retries"
...
# ── Main ───────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="VTU Content Generator - MiniMax Parallel")
